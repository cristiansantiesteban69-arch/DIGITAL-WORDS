...
function windowPosition(dummy, side, floorIndex, slot, perSide) {
  ...
  const y = floorIndex * FLOOR_HEIGHT + FLOOR_HEIGHT * 0.5;
...
/**
 * Windows
 * A single InstancedMesh for every window on the tower. Each window has a
 * seeded "on" threshold along `windowsT` (0-1), so they light up
 * staggered rather than all at once, plus a small chance of never
 * lighting at all — a building that feels lived-in, not a light switch.
 * Color mixes mostly cool tones with occasional warm accents, staying
 * distinct from the crane/site's saturated construction amber per the
 * project's cold-city / warm-site / elegant-building lighting rule.
 * `lowDetail` reduces windows-per-side (3 -> 2) on small/low-power devices.
 */
